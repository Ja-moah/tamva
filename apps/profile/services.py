...
from apps.connector.models import InstitutionConnection
from apps.ledger.models import Account, LedgerEntry
from apps.partner.models import Institution
...
def _profile_for_tenant(*, customer_id: UUID, institution: Institution) -> FinancialProfile:
    ...
    return FinancialProfile.objects.get_or_create(
        institution=institution, customer_id=customer_id
    )[0]


def _entries_for_window(
    *, customer_id: UUID, institution: Institution, start, end
) -> QuerySet[LedgerEntry]:
    return LedgerEntry.objects.select_related("account", "posting").filter(
...
def _fingerprint(entries: list[LedgerEntry], *, start, end, version: str) -> str:
    values = [
        {
            "id": str(entry.id),
...
def compute_profile(
    *,
    ...
    savings_rate = (
        (net / total_inflows).quantize(Decimal("0.0001"))
        if total_inflows
        else Decimal("0")
    )
